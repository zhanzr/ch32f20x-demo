...
/* --------------------------------------------------------------------------
 * e_server front-end (CH32F207VCT6): single page, three tabs, no external
 * libraries. Talks to the embedded C backend over relative /api paths.
 * ------------------------------------------------------------------------ */
...
async function setLed(i, on) {
  ...
  try {
    ...
    loadLeds();                   /* re-sync checkbox with the server */
  } finally {
    ledBusy = false;
  }
}
